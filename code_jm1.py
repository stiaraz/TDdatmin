...
# -*- coding: utf-8 -*-
"""
Created on Tue Oct 29 22:11:24 2019
...
dataset= pd.read_csv('dataset/jm1.csv', skiprows = range(0,355), header = None)
...
dataset= pd.read_csv('../dataset/jm1.csv', skiprows = range(0,355), header = None)
dataset=pd.DataFrame(dataset.replace('?', 0))
#dts=dataset.fillna(0)
dts = dataset.groupby(dataset.columns, axis = 1).transform(lambda x: x.fillna(x.mean()))
kelas= dts[21]
print('Before: Class{}'. format(Counter(kelas)))
df = dts.copy()
del df[21]
...
# method='hessian', eigen_solver='dense'
>>>>>>> f9a22f349a5dda238e07e5c4e6ddc6cf10f36ece
