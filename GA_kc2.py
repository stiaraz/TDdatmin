# -*- coding: utf-8 -*-
"""
Created on Tue Nov  5 15:31:17 2019
...
def fitness(x):
    ...
    clf = svm.SVC(C= x, kernel='rbf',gamma = 10, tol=0.001, max_iter = -1)
    svm_fitness = 0
...
#            print "loop = "+str(x)
        ...
        n_fold = 10
        skf = StratifiedKFold(n_splits = n_fold )
        clf = svm.SVC(C= Krom[k_fixed], kernel='rbf',gamma = 10, tol=0.001, max_iter = -1)
        svm_accuracy = 0
        svm_recall_yes = 0
        svm_recall_no = 0
        svm_precision_yes = 0
        ...
        for train_index, test_index in skf.split(dataset_minmax, class_data):
            ...
            svm_recall_no += recallno
            svm_precision_yes += precisionyes
            svm_precision_no += precisionno
...
#            print("\nAkurasi : " + str(akurasi) +"%")
            ...
            csvmtrain = metrics.confusion_matrix(y_true_svm, y_pred_svm)
        print('Akurasi Support Vector Machine : ' + repr(svm_accuracy / n_fold))
        print('Recal kelas yes Support Vector Machine : ' + repr(svm_recall_yes / n_fold))
        print('Recal kelas no Vector Machine : ' + repr(svm_recall_no / n_fold))
        print('Presisi kelas yes Support Vector Machine : ' + repr(svm_precision_yes / n_fold))
        print('Presisi kelas no Vector Machine : ' + repr(svm_precision_no / n_fold))
